...
bool sumasActuales(int& i, int& j, int& elem){
    ...
    if ((j == N-1) && (fila_suma[i]+elem != numero_magico)){return false;}
    if ((i == N-1) && (col_suma[j]+elem) != numero_magico){return false;}
...
void cuadradoMagicoK(vector<vector<int>>& C, int i, int j){
    ...
    if (j == N){
        ...
        if (i == N-1){
            ...
            if (encontrado){cuadradoK = C; return;}
        } else {
        ...
        for (int k = 1; k <= N * N; k++){
            ...
            if (!sumasActuales(i, j, k)){ continue;}
